...
export default function TagManagerPage() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-semibold font-display mb-2 text-[color:var(--ink)]">标签管理</h1>
                <p className="text-[color:var(--ink-muted)]">创建、查看与删除标签，便于文章归类与检索。</p>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.2fr_360px]">
                <Card className="border border-[color:var(--card-border)] bg-[color:var(--paper-soft)] shadow-[0_26px_50px_-40px_rgba(31,41,55,0.35)]">
                    <CardHeader>
                        <CardTitle className="text-[color:var(--ink)]">标签列表</CardTitle>
                    </CardHeader>
                    <CardContent>
                        {tagsQuery.isLoading && <p className="text-sm text-[color:var(--ink-soft)]">加载中...</p>}
                        {!tagsQuery.isLoading && (
                            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
                                    ...
                                    >
                                        <div className="flex items-center gap-2 min-w-0">
                                            <span className="h-8 w-8 rounded-full bg-[color:var(--paper-strong)] flex items-center justify-center">
                                                <Tag className="h-4 w-4 text-[color:var(--ink-soft)]" />
                                            </span>
                                            <span className="truncate font-medium text-[color:var(--ink)]">{tag.name}</span>
                                        </div>
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                ...
                                {!tags.length && (
                                    <div className="col-span-full text-sm text-[color:var(--ink-soft)]">暂无标签</div>
                                )}
                            </div>
                        )}
                    </CardContent>
                ...
                <Card className="border border-[color:var(--card-border)] bg-[color:var(--paper-soft)] shadow-[0_26px_50px_-40px_rgba(31,41,55,0.35)]">
                    <CardHeader>
                        <CardTitle className="text-[color:var(--ink)]">新建标签</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            <div className="space-y-2">
                                <label className="text-sm text-[color:var(--ink-muted)]">标签名称</label>
                                <Input
                                    required
                                    value={name}
